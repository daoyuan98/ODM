...
from memory_bank import MemoryBanks
...
class InteractionHead(Module):
    ...
    def __init__(self,
        ...
        box_pair_suppressor: Module,
        box_pair_predictor: Module,
        box_pair_balanced_predictor: Module,
        # Dataset properties
        human_idx: int,
        num_classes: int,
        # Hyperparameters
        ...
        self.box_pair_balanced_predictor = box_pair_balanced_predictor
        # self.box_pair_negative_predictor = box_pair_negative_predictor
        ...
        self.memory_bank = MemoryBanks(buffer_size, 2048, age_weight=args.age_weight, freq_path=freq_path)
        self.K = K
        self.args = args
        self.max_iter = 0
    ...
    def postprocess(self,
        ...
        if logits_base is not None:
            logits_base_list = logits_base.split(num_boxes)
            scores_base = torch.sigmoid(logits_base)

        if logits_balance is not None:
            logits_balance_list = logits_balance.split(num_boxes)
            scores_balance = torch.sigmoid(logits_balance)

        weights = weights.split(num_boxes)
        scores = scores.split(num_boxes)
        if logits_base is not None:
            scores_base = scores_base.split(num_boxes)
        if logits_balance is not None:
            scores_balance = scores_balance.split(num_boxes)
        ...
        results = []
        xs = []; ys = []
        for i, (w, s, p, b_h, b_o, o, l) in enumerate(zip(
            ...
            result_dict = dict(
                ...
                index=x, prediction=y, logits=logits[x, y], original_scores=s, original_labels=l,
                scores=s[x, y] * p[:, x, y].prod(dim=0) * w[x].detach(),
                object=o, prior=p[:, x, y], weights=w
            )

            if logits_base is not None:
                result_dict["scores_base"] = scores_base[i][x, y] * p[:, x, y].prod(dim=0) * w[x].detach()
                result_dict["logits_base"] = logits_base_list[i][x, y]
            if logits_balance is not None:
                result_dict["scores_balance"] = scores_balance[i][x, y] * p[:, x, y].prod(dim=0)* w[x].detach()
                result_dict["logits_balance"] = logits_balance_list[i][x, y]
    ...
    def retrieve_pos(self, feature, labels, object_labels):
        assert len(feature) == len(labels) == len(object_labels)
        idx, = torch.where(labels.sum(dim=-1) > 0)
        return feature[idx, :], labels[idx, :], object_labels[idx]


    def select_valid(self, logits, objects, counts):
        scores = torch.sigmoid(logits)
        scores = scores.split(counts)

        score_list = []
        for score, obj in zip(scores, objects):
            index = obj
            idx = torch.tensor(self.box_pair_head.object_class_to_target_class[index]).to(score.device)
            score = score[:, idx.long()]
            score_list.append(score.flatten())

        return torch.cat(score_list)


    def consistency_loss(self, src, tar):
        # TODO: ugly
        return nn.MSELoss()(src, tar)


    def pred_loss(self, scores, labels):
        return F.binary_cross_entropy(scores, torch.cat(labels))


    def sel_pos_label(self, labels, pos_object_class):
        ret_list = []
        for i, o in enumerate(pos_object_class):
            idx = o.item()
            ret_list.append(labels[i, self.box_pair_head.object_class_to_target_class[idx]].flatten())
        return ret_list

    def select_valid_logits(self, pred, obj):
        n_class = pred.shape[1]

        idx_list = []
        for i, o in enumerate(obj):
            idx = o.item()
            valid_idx = np.array(self.box_pair_head.object_class_to_target_class[idx]) + i * n_class
            idx_list.append(valid_idx)

        idx = np.concatenate(idx_list)
        return pred.flatten()[idx]
    ...
    def forward(self,
        ...
        self.max_iter = max(self.max_iter, n_iter)
        ...
        if self.training:
            results, xs, ys = self.postprocess(
                logits_p, logits_s, box_pair_prior,
                boxes_h, boxes_o,
                object_class, box_pair_labels
            )
            pos_box_pair_features, pos_box_pair_labels, pos_object_class = \
                self.retrieve_pos(box_pair_features, torch.cat(box_pair_labels), torch.cat(object_class))
            sel_pos_label = self.sel_pos_label(pos_box_pair_labels, pos_object_class)
            if n_epoch >= self.args.start_balance_epoch:

                buffer_feat, buffer_label, buffer_obj_label, buffer_counts = \
                    self.memory_bank.read(pos_box_pair_features, sel_pos_label, pos_object_class, self.K)
                
                if buffer_feat is not None:
                    buffer_logits = self.box_pair_balanced_predictor(buffer_feat)
                    selected_buffer_logits = self.select_valid(buffer_logits, buffer_obj_label, buffer_counts)
                    pred_loss = self.args.balance_classifier_weight * self.pred_loss(selected_buffer_logits, buffer_label)
                else:
                    pred_loss = 0.
                consistency_loss = 0.

                # write in positive data of this batch
                self.memory_bank.write(pos_box_pair_features, sel_pos_label, pos_object_class, n_epoch, n_iter)
            ...
            loss_dict = dict(
                ...
                buffer_loss=pred_loss,   # score level
                consistency_loss=consistency_loss # logits level
            )
            results.append(loss_dict)

            
        else:
            # the average of the two predictor for evaluation
            logits_balance = self.box_pair_balanced_predictor(box_pair_features)
            lam = self.args.lam
            logits = logits_p * lam +  logits_balance * (1 - lam)

            results, xs, ys = self.postprocess(
                logits, logits_s, box_pair_prior,
                boxes_h, boxes_o,
                object_class, box_pair_labels,
                logits_p, logits_balance
            )
...
class GraphHead(Module):
    ...
    def compute_prior_scores(self,
        ...
        for obj in object_class[y]:
            index = obj.item()
            target_cls_idx.append(self.object_class_to_target_class[index])
    ...
    def forward(self,
        ...
        for b_idx, (coords, labels, scores) in enumerate(zip(box_coords, box_labels, box_scores)):
            ...
            h_node_encodings = node_encodings[:n_h] # n X 1024
